...
export class PushNotificationService {
  ...
  async sendCriticalAlert(
    ...
    try {
      ...
      if (alert.deviceId !== undefined) {
        data.deviceId = alert.deviceId.toString();
      }
      ...
      for (const token of activeTokens) {
        try {
          ...
          console.log(
            `Push notification sent to token: ${token.token.substring(0, 10)}...`
          );
        } catch (error: any) {
          console.error(
            `Failed to send to token ${token.token.substring(0, 10)}...:`,
            error
          );
    ...
    try {
      ...
      for (const level of waterLevels) {
        if (level.level > 80) {
          ...
          alerts.push({
            type: "water_level",
            message: `High water level detected: ${level.level}% at ${
      ...
      for (const bin of wasteBins) {
        if (bin.fullness > 85) {
          // Full waste bin threshold
          const device = devices.find((d) => d.id === bin.deviceId);
          alerts.push({
            type: "waste_bin",
            message: `Waste bin is ${bin.fullness}% full at ${
              device?.name || "Unknown Device"
            }`,
      ...
      for (const device of devices) {
        if (device.lastSeen) {
          ...
          if (hoursSinceLastSeen > 2) {
            // Device offline for more than 2 hours
            alerts.push({
              type: "device_offline",
              message: `Device ${
                device.name
              } has been offline for ${Math.round(
                hoursSinceLastSeen
              )} hours`,
